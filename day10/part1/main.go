...
func find_orders(lines [][]int, trailheads []position) []int {
	...
	for _, trailhead := range trailheads {
		all_surrounding := map[position]bool{
			trailhead: true,
		}
		for i := 1; i <= 9; i++ {
			new_surrounding := map[position]bool{}
			for location := range all_surrounding {
				next_location := find_surrounding(lines, location, i)
				for new_location := range next_location {
					new_surrounding[new_location] = true
				}
			}
			all_surrounding = new_surrounding
		}
		orders = append(orders, len(all_surrounding))
	}
	return orders
}

func find_surrounding(lines [][]int, location position, next_number int) map[position]bool {
	surrounding_locations := map[position]bool{}
	if location.y-1 >= 0 && lines[location.y-1][location.x] == next_number {
		surrounding_locations[position{location.x, location.y - 1}] = true
	}
	...
	if location.x-1 >= 0 && lines[location.y][location.x-1] == next_number {
		surrounding_locations[position{location.x - 1, location.y}] = true
	}
